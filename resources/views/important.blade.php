...
@section('content')
    <h1 class="text-4xl text-cyan-700 font-bold"><i class="fa-regular fa-star mr-3"></i> Important</h1>
    ...
    <div>
        @foreach ($data as $value)
            <div class="mb-3 w-full bg-white py-6 px-5 rounded text-cyan-500 flex justify-between">
                <div>
                    <p><a href="/important/done/{{$value->id}}"><i class="fa-regular fa-circle mr-3"></i></a> {{ $value->name }}</p>
                </div>
                <div>
                    <p class="font-light">{{ \Carbon\Carbon::parse($value->date)->format('l, F j') }}
                        <a href="/important/fav/{{ $value->id }}">
                            @if ($value->is_fav)
    ...
    <div>
        @foreach ($done as $value)
            <div class="mb-3 w-full bg-white py-6 px-5 rounded text-cyan-500 flex justify-between">
                <div>
                    <p><a href="/important/done/{{$value->id}}"><i class="fa-regular fa-circle-dot mr-3"></i></a> {{ $value->name }}</p>
                </div>
                <div>
                    <p class="font-light">{{ \Carbon\Carbon::parse($value->date)->format('l, F j') }}
                        <a href="/important/fav/{{ $value->id }}">
    ...
    <div class="flex justify-center">
        <a href="/important/create" class="w-9/12 fixed bottom-5">
            <button
