...
@section('content')
    ...
    <div>
        @foreach ($data as $value)
            <div class="mb-3 w-full bg-white py-6 px-5 rounded text-cyan-500 flex justify-between">
                <div>
                    <p><a href="/task/done/{{$value->id}}"><i class="fa-regular fa-circle mr-3"></i></a> {{ $value->name }}</p>
                </div>
                <div>
                    <p class="font-light">{{ \Carbon\Carbon::parse($value->date)->format('l, F j') }}
                        <a href="/task/fav/{{ $value->id }}">
                            @if ($value->is_fav)
    ...
    <div>
        @foreach ($done as $value)
            <div class="mb-3 w-full bg-white py-6 px-5 rounded text-cyan-500 flex justify-between">
                <div>
                    <p><a href="/task/done/{{$value->id}}"><i class="fa-regular fa-circle-dot mr-3"></i></a> {{ $value->name }}</p>
                </div>
                <div>
                    <p class="font-light">{{ \Carbon\Carbon::parse($value->date)->format('l, F j') }}
                        <a href="/task/fav/{{ $value->id }}">
    ...
    <div class="flex justify-center">
        <a href="/task/create" class="w-9/12 fixed bottom-5">
            <button
